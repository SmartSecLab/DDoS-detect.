...
df = pd.read_csv("labeled_db.csv", parse_dates=["Timestamp"])

# Define the desired order of features within each bar group
desired_feature_order = ['CPU_usage', 'Interrupts_per_sec', 'Num_processes', 'RAM-percentage', 'DSK-write', 'DSK-read']
...
df[features] = df[features].apply(lambda x: (x - x.min()) / (x.max() - x.min()) * 100)
...
for i, attack_type in enumerate(df['Attack_Type'].unique()):
    ...
    bars = ax.bar(i + np.arange(len(features)) * bar_width, attack_data[desired_feature_order].mean(), width=bar_width, color=feature_colors, alpha=0.7)
    ...
    ax.errorbar(i + np.arange(len(features)) * bar_width + bar_width / 2,
                ...
                yerr=[attack_data[feature].median() - attack_data[feature].quantile(0.25) for feature in desired_feature_order],
                fmt='o', color='black', markersize=8, capsize=5, label='_nolegend_')

# Create a legend with consistent colors and feature labels
ax.legend(bars, desired_feature_order, title='Features', loc='upper left', bbox_to_anchor=(1, 1))

ax.set_ylabel('Percentage of Feature Values')
ax.set_ylim(0, 100)  # Set Y-axis limit to 0-100%
plt.xlabel('Traffic Type')
plt.xticks(np.arange(len(df['Attack_Type'].unique())) + (bar_width * (len(features) - 1) / 2), [attack_type_mapping.get(att_type.lower(), att_type.title()) for att_type in df['Attack_Type'].unique()])
